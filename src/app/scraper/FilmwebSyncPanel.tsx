  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm shadow-sm dark:border-white/10 dark:bg-white/5 dark:shadow-none">
          ...
          {status.lastSync ? (
            <span className="text-slate-600 dark:text-slate-300">
              Ostatnia sync: <strong className="text-slate-900 dark:text-white">{status.lastSync}</strong>
              {" · "}{status.totalItems} pozycji w bazie scrapera
            </span>
          ) : (
            <span className="text-slate-500 dark:text-slate-400">Brak poprzedniej synchronizacji</span>
          )}
        </div>
      )}
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-white/5 dark:shadow-none">
        <h2 className="mb-4 text-sm font-semibold text-slate-900 dark:text-white">Dane logowania Filmweb</h2>
        <div className="space-y-3">
          <div>
            <label className="mb-1 block text-xs text-slate-500 dark:text-slate-400">E-mail</label>
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} disabled={running}
              placeholder="twoj@email.pl"
              className="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-900 placeholder-slate-400 focus:border-emerald-400/50 focus:outline-none disabled:opacity-50 dark:border-white/10 dark:bg-slate-900 dark:text-white dark:placeholder-slate-500" />
          </div>
          <div>
            <label className="mb-1 block text-xs text-slate-500 dark:text-slate-400">Hasło</label>
            <div className="relative">
              <input type={showPass ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} disabled={running}
                placeholder="••••••••"
                className="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 pr-10 text-sm text-slate-900 placeholder-slate-400 focus:border-emerald-400/50 focus:outline-none disabled:opacity-50 dark:border-white/10 dark:bg-slate-900 dark:text-white dark:placeholder-slate-500" />
      ...
      {running && (
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-white/5 dark:shadow-none">
          ...
          <div className="mb-3 h-2 overflow-hidden rounded-full bg-slate-200 dark:bg-slate-800">
            <div className="h-full rounded-full bg-emerald-500 transition-all duration-300 dark:bg-emerald-400" style={{ width: `${percent}%` }} />
          </div>
      ...
      {done && !running && (
        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5 dark:border-emerald-400/20 dark:bg-emerald-400/5">
          ...
          <div className="mt-3 text-xs text-slate-500 dark:text-slate-400">
            Łącznie w bazie: <strong className="text-slate-900 dark:text-white">{(done as any).totalItems}</strong> pozycji
            {(done as any).uploadResult?.batchId && (
              <> · <a href={`/import/${(done as any).uploadResult.batchId}`} className="text-emerald-600 hover:underline dark:text-emerald-300">
                Otwórz import →
              </a></>
            )}
          </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-slate-50 dark:border-white/10 dark:bg-slate-950">
          ...
          {showLogs && (
            <div className="h-48 overflow-y-auto border-t border-slate-200 p-4 font-mono text-xs text-slate-500 dark:border-white/10 dark:text-slate-400">
              {logs.map((log, i) => (
                <div key={i} className={`mb-0.5 ${log.phase === "error" ? "text-red-600 dark:text-red-400" : log.phase === "done" ? "text-emerald-600 dark:text-emerald-400" : ""}`}>
                  <span className="text-slate-400 dark:text-slate-600">[{log.phase}]</span> {log.message}
                </div>
              ))}
              <div ref={logsEndRef} />
            </div>
