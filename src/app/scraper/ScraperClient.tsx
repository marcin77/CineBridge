  ...
  <div className="mx-auto max-w-4xl px-6 py-10">
    ...
    <div className="mb-6 flex items-start gap-3 rounded-2xl border border-emerald-400/20 bg-emerald-400/5 p-5">
      ...
      <section className="mb-8 rounded-2xl border border-white/10 bg-white/5 p-6">
        <h2 className="mb-4 text-base font-semibold text-white">Jak uruchomić scraper?</h2>
        <ol className="space-y-3 text-sm text-slate-300">
          {[
            <>
              ...
              >
                filmweb.pl <ExternalLink size={12} />
              </a>{" "}
              i upewnij się, że jesteś <strong className="text-white">zalogowany</strong> na swoje konto.
            </>,
            <>
              Naciśnij <kbd className="rounded bg-white/10 px-1.5 py-0.5 text-xs font-mono text-white">F12</kbd>{" "}
              (lub <kbd className="rounded bg-white/10 px-1.5 py-0.5 text-xs font-mono text-white">Ctrl+Shift+J</kbd>) aby otworzyć{" "}
              <strong className="text-white">DevTools → Console</strong>.
            </>,
            <>
              Skopiuj cały skrypt poniżej (przycisk <strong className="text-white">Kopiuj skrypt</strong>)
              i wklej go w konsoli, następnie naciśnij{" "}
              <kbd className="rounded bg-white/10 px-1.5 py-0.5 text-xs font-mono text-white">Enter</kbd>.
            </>,
            <>
              Skrypt automatycznie pobierze Twoje dane (oceny, komentarze, listy) i ściągnie plik{" "}
              <code className="rounded bg-white/10 px-1 text-xs text-white">filmweb_export_YYYY-MM-DD.csv</code>.
            </>,
            <>
              <strong className="text-white">Wgraj pobrany plik</strong> na stronie{" "}
              <a href="/import" className="text-emerald-300 hover:underline">
                Import
              </a>{" "}
            ...
            <li key={i} className="flex items-start gap-3">
              <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-emerald-400/20 text-[11px] font-bold text-emerald-300 mt-0.5">
                {i + 1}
              </span>
              <span>{step}</span>
            </li>
      ...
      {/* Warning */}
      <div className="mb-6 flex items-start gap-3 rounded-xl border border-amber-400/30 bg-amber-400/10 px-4 py-3 text-sm text-amber-200">
        <AlertTriangle size={16} className="mt-0.5 shrink-0 text-amber-400" />
        <div>
          <strong className="text-amber-300">Uwaga:</strong> Skrypt działa bezpośrednio w Twojej
          przeglądarce i pobiera dane tylko z Twojego konta. Nigdy nie wklejaj skryptów
          z nieznanych źródeł w konsolę przeglądarki.
        </div>
      </div>

      {/* Info about editability */}
      <div className="mb-6 flex items-start gap-3 rounded-xl border border-sky-400/20 bg-sky-400/5 px-4 py-3 text-sm text-sky-200">
        <Info size={16} className="mt-0.5 shrink-0 text-sky-400" />
        <div>
          Skrypt jest w{" "}
          <strong className="text-sky-300">jednym pliku</strong> z wyraźnie wyodrębnioną
          sekcją <code className="rounded bg-white/10 px-1 text-xs text-white">FILMWEB_CONFIG</code>.
          Jeśli Filmweb zmieni strukturę swojego API, wystarczy zaktualizować URLe i nazwy pól
          w tej sekcji — reszta logiki pozostaje bez zmian. Plik źródłowy:{" "}
          <code className="rounded bg-white/10 px-1 text-xs text-white">
            public/filmweb-scraper/filmweb-scraper.js
          </code>
          .
        </div>
      </div>
      ...
      {/* Script display */}
      <section className="rounded-2xl border border-white/10 bg-slate-900/80">
        <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1 rounded-lg border border-white/10 px-3 py-1.5 text-xs text-slate-400 transition hover:bg-white/5 hover:text-white"
            >
        ...
        {showRaw && (
          <div className="relative max-h-[60vh] overflow-auto">
            <pre className="p-4 text-xs leading-relaxed text-slate-300 font-mono whitespace-pre">
              {scriptContent}
            </pre>
          </div>
        )}
        ...
        {!showRaw && (
          <div className="px-4 py-6 text-center">
            <p className="text-sm text-slate-500">
              Kliknij <strong className="text-slate-300">Kopiuj skrypt</strong> aby skopiować do schowka,
              lub <strong className="text-slate-300">Rozwiń</strong> aby podejrzeć kod.
            </p>
          </div>
        )}
      </section>
      ...
      <section className="mt-8 rounded-2xl border border-white/10 bg-white/5 p-6">
        <h2 className="mb-4 text-base font-semibold text-white">Co zbiera skrypt?</h2>
      ...
      <section className="mt-6 rounded-2xl border border-white/10 bg-white/5 p-6">
        <h2 className="mb-4 text-base font-semibold text-white">Obsługiwane formaty importu</h2>
        ...
        <ul className="space-y-2 text-sm">
          {[
            {
              name: "CineBridge Scraper (ten skrypt)",
              fmt: "filmweb_export_YYYY-MM-DD.csv",
              badge: "Zalecane",
              badgeColor: "bg-emerald-400/20 text-emerald-300",
            },
            {
              name: "filmweb-export (Python)",
              fmt: "CSV / JSON",
              badge: "Obsługiwane",
            ...
            {
              name: "Filmweb2Letterboxd",
              fmt: "CSV (format Letterboxd)",
              badge: "Obsługiwane",
              badgeColor: "bg-sky-400/20 text-sky-300",
              url: "https://github.com/JSerwatka/Filmweb2Letterboxd",
            },
            {
              name: "Własny plik CSV",
              fmt: "Kolumny: title, year, user_rating, watched_at, comment…",
              badge: "Elastyczne",
            ...
            <li
              key={item.name}
              className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-white/10 bg-slate-900/40 px-4 py-3"
            >
              <div>
                ...
                <div className="text-xs text-slate-400">{item.fmt}</div>
              </div>
